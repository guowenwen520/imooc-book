    ...
    this.task = setInterval(() => {
      this.$refs.mask.forEach((item, index) => {
        ...
        if (index === 0) {
          ...
          if (this.add && maskWidth.value < 16) {
            const perMaskWidth = this.maskWidth[index - 1]
            if (perMaskWidth.value >= 8) {
              maskWidth.value++
              lineWidth.value--
            }
          } else if (!this.add && lineWidth.value < 16) {
            const perLineWidth = this.lineWidth[index - 1]
            if (perLineWidth.value >= 8) {
              maskWidth.value--
              lineWidth.value++
            }
          }
